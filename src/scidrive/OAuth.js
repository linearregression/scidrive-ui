...
lang.getObject("scidrive.OAuth", true);

scidrive.OAuth = new (function(){
	//	summary:
	...
	function getOAuthParams(/* dojox.io.__OAuthArgs */oaa){
		var o = [
			...
			if(oaa.token){
				o.push(["oauth_token", oaa.token.key]);
			}
		return o;
	}
	...
	function convertArgs(args){
		...
		if(map.query){
			...
			//for(var p in tmp){ tmp[p] = encodeURIComponent(tmp[p]); } // by dimm - seems to fix error with params with spaces
			miArgs.push(tmp);
		}
		args._url = map.url;
	...
	function baseString(/* String */method, /* dojo.__XhrArgs */args, headerParams, /* dojox.io.__OAuthArgs */oaa){
		convertArgs(args);

		var a = args._parameters;

		var params = lang.clone(headerParams);

		if(a+"" != "") {
			array.forEach(a, function(entry, i){
				params.push(entry);
			});
		}
		...
		var h = dojo.map(params, function(item){
